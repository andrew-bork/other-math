...
/** @type {Object.<string, {type: number}>} */
const variables = {};

function defineFunction(name, expression, params = ["x"]) {
    variables[name] = {
        name: name,
        type: 1,
...
function defineEvalFunction(name, eval, derivative, inverse) {
    variables[name] = {
        name: name,
        type: 0,
        evaluate: eval,
...
function defineVariable(name, value, redefinable = true, instanteval = true) {
    variables[name] = {
        name: name,
        type: 2,
        value: value,
...
defineEvalFunction("sin", (value) => {
    return num(Math.sin(value.val.r));
});
defineEvalFunction("cos", (value) => {
    return num(Math.cos(value.val.r));
});
defineEvalFunction("ln", (value) => {
    return num(math.complex.ln(value.val));
})
variables["sin"].derivative = {
...
defineVariable("i", math.complex.imag(1), false, true);
